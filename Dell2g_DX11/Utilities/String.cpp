...
///@return 변환 완료 문자열
//////////////////////////////////////////////////////////////////////////
string CString::ToUpper(string InValue)
{
	string temp = InValue;
	std::transform(InValue.begin(), InValue.end(), temp.begin(), ::toupper);

	return temp;
}
wstring CString::ToUpper(wstring InValue)
{
	wstring temp = InValue;
	std::transform(InValue.begin(), InValue.end(), temp.begin(), ::toupper);
...
///@return 변환 완료 문자열
//////////////////////////////////////////////////////////////////////////
string CString::ToLower(string InValue)
{
	string temp = InValue;
	std::transform(InValue.begin(), InValue.end(), temp.begin(), ::tolower);

	return temp;
}

wstring CString::ToLower(wstring InValue)
{
	wstring temp = InValue;
	std::transform(InValue.begin(), InValue.end(), temp.begin(), ::tolower);
...
FVector2D CString::FromVector2D(const string& InValue)
{
	vector<string> v;
	SplitString(&v, InValue, ",");

	return FVector2D(stof(v[0]), stof(v[1]));
}

FVector CString::FromVector(const string& InValue)
{
	vector<string> v;
	SplitString(&v, InValue, ",");

	return FVector(stof(v[0]), stof(v[1]), stof(v[2]));
}

FVector4 CString::FromVector4(const string& InValue)
{
	vector<string> v;
	SplitString(&v, InValue, ",");

	return FVector4(stof(v[0]), stof(v[1]), stof(v[2]), stof(v[3]));
}
